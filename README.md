...
1. Power Management (Do this first!)

Connect the LiPo battery to the Buck Converter IN+ and IN-.

Turn the screw on the converter until the OUT reads exactly 5.0V on a multimeter.

Disconnect the battery. Wire the converter's OUT+ to the ESP32 VIN (or 5V) and OUT- to ESP32 GND.

2. Drivetrain & Driver Power

Wire the positive terminals of both Left motors together, and negative terminals together. Connect them to M+ and M- on the Left BTS7960.

Repeat the same parallel wiring for the Right motors and connect to the Right BTS7960.

Connect the B+ and B- of both motor drivers directly to the LiPo battery.

Crucial: Connect the battery's negative terminal (Ground) to a GND pin on the ESP32.

3. Logic Wiring (Matching the Code)
